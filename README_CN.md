...
推理更长不一定更好。“长度扩展”可能引入更隐蔽且难以定位的错误与幻觉。ReasoningLens 的 **Agentic Diagnosis** 将推理轨迹作为可审计对象，形成“诊断-校验-修复”的闭环流程：

- **⚡ 分批分析：** 高效处理超长轨迹，兼顾规模与上下文连续性。
- **🧠 滚动摘要记忆：** 记住前文关键状态，识别跨段落的逻辑漂移与不一致。
- **🧮 工具增强校验：** 集成计算器自动核验算术步骤，减少“低级数学错误”漏检。
- **🛠️ 错误修复建议：** 不止于发现问题，系统还会生成可执行的修复建议（如修正中间假设、替换无效变换、对特定子步骤触发工具复核），帮助开发者从“错误定位”更快推进到“定向修复”。
